window.addEventListener('load', () => {
  ...
  function buildCard(key, r, isMatch) {
    ...
    const icon = key === 'spring' ? '😊' : key === 'summer' ? '🌊' : key === 'autumn' ? '🍂' : '❄️';
    const seasonName = r.typename.split(' · ')[1];

    return `
      <div class="swiper-slide pc-swiper-slide" data-type="${key}">
        <div class="pc-swiper-card is-${key}">
          ${matchBadge}
          <div class="pc-card__band">
            <div class="pc-card__band-inner">
              <span class="pc-card__band-icon">${icon}</span>
              <span class="pc-card__band-title">${seasonName}</span>
            </div>
          </div>
